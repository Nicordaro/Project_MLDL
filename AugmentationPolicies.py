...
class SubPolicy(object):
    "Each SubPolicy is a combination of two transformations with their own probability and intensity"
    def __init__(self, p1, operation1, magnitude_idx1, p2, operation2, magnitude_idx2, fillcolor=(128, 128, 128)):

       #Define the domains of intensity for each function
        ranges = {
            "shearX": np.linspace(-0.3,0.3, 11),
            "shearY": np.linspace(-0.3,0.3, 11),
            "translateX": np.linspace( -150 / 331, 150 / 331, 11),
            "translateY": np.linspace(-150 / 331, 150 / 331, 11),
            "rotate": np.linspace(-30, 30, 11),
            "color": np.linspace(0.1, 1.9, 11),
            "posterize": np.round(np.linspace(4, 8, 11), 0).astype(np.int),
    ...
    def __call__(self, img):
      # Apply the transformations if the random number is below or equal to the probability of the transformations
        if random.uniform(0,1) <= self.p1: img = self.operation1(img, self.magnitude1)
        if random.uniform(0,1) <= self.p2: img = self.operation2(img, self.magnitude2)
        return img
